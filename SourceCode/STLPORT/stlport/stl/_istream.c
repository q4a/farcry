...
#else
#define __BIS_int_type__                                                       \
  _STLP_TYPENAME_ON_RETURN_TYPE basic_istream<_CharT, _Traits>::int_type
#define __BIS_pos_type__                                                       \
  _STLP_TYPENAME_ON_RETURN_TYPE basic_istream<_CharT, _Traits>::pos_type
#define __BIS_off_type__                                                       \
  _STLP_TYPENAME_ON_RETURN_TYPE basic_istream<_CharT, _Traits>::off_type
#endif
...
template <class _Traits> struct _Is_not_wspace {
...
template <class _Traits> struct _Is_wspace_null {
  typedef typename _Traits::char_type argument_type;
  typedef bool result_type;
...
template <class _Traits> struct _Scan_for_wspace {
  ...
  const char_type *operator()(const char_type *__first,
                              const char_type *__last) const {
    return _M_ctype->scan_is(ctype_base::space, __first, __last);
  }
};

template <class _Traits> struct _Scan_wspace_null {
  typedef typename _Traits::char_type char_type;
  typedef char_type *first_argument_type;
  typedef char_type *second_argument_type;
  typedef char_type *result_type;
  ...
  const char_type *operator()(const char_type *__first,
                              const char_type *__last) const {
    __last = find_if(__first, __last, _Eq_char_bound<_Traits>(char_type()));
    return _M_ctype->scan_is(ctype_base::space, __first, __last);
  }
};

template <class _Traits> struct _Scan_for_not_wspace {
  typedef typename _Traits::char_type char_type;
  typedef char_type *first_argument_type;
  typedef char_type *second_argument_type;
  typedef char_type *result_type;
...
template <class _Traits> struct _Scan_for_char_val {
  typedef typename _Traits::char_type char_type;
  typedef char_type *first_argument_type;
  typedef char_type *second_argument_type;
  typedef char_type *result_type;
...
template <class _Traits> struct _Scan_for_int_val {
  typedef typename _Traits::char_type char_type;
  ...
  const char_type *operator()(const char_type *__first,
                              const char_type *__last) const {
    return find_if(__first, __last, _Eq_int_bound<_Traits>(_M_val));
  }
};
  ...
  if (__sentry) {
    ...
    _STLP_TRY {
      ((const _Num_get &)use_facet<_Num_get>(__that.getloc()))
          .get(istreambuf_iterator<_CharT, _Traits>(__that.rdbuf()), 0, __that,
               __err, __val);
    }
    _STLP_CATCH_ALL { __that._M_handle_exception(ios_base::badbit); }
    if (__err)
...
__BIS_int_type__ basic_istream<_CharT, _Traits>::peek() {
  ...
  if (__sentry) {
    _STLP_TRY { __tmp = this->rdbuf()->sgetc(); }
    _STLP_CATCH_ALL { this->_M_handle_exception(ios_base::badbit); }
...
__BIS_int_type__ basic_istream<_CharT, _Traits>::get() {
  ...
  if (__sentry) {
    _STLP_TRY { __tmp = this->rdbuf()->sbumpc(); }
    _STLP_CATCH_ALL { this->_M_handle_exception(ios_base::badbit); }
...
basic_istream<_CharT, _Traits> &basic_istream<_CharT, _Traits>::ignore() {
  ...
  if (__sentry) {
    int_type __c;
    _STLP_TRY { __c = this->rdbuf()->sbumpc(); }
    _STLP_CATCH_ALL {
...
basic_istream<_CharT, _Traits>::putback(_CharT __c) {
  ...
  if (__sentry) {
    ...
    if (__buf) {
      _STLP_TRY { __tmp = __buf->sputbackc(__c); }
      _STLP_CATCH_ALL { this->_M_handle_exception(ios_base::badbit); }
    }
    if (this->_S_eof(__tmp))
...
__BIS_pos_type__ basic_istream<_CharT, _Traits>::tellg() {
  //   sentry __sentry(*this, _No_Skip_WS());
  basic_streambuf<_CharT, _Traits> *__buf = this->rdbuf();
  return (__buf && !this->fail())
             ? __buf->pubseekoff(0, ios_base::cur, ios_base::in)
             : pos_type(-1);
}
...
_M_read_unbuffered(basic_istream<_CharT, _Traits> *__that,
                   basic_streambuf<_CharT, _Traits> *__buf, streamsize _Num,
                   _CharT *__s, _Is_Delim __is_delim, bool __extract_delim,
                   bool __append_null, bool __is_getline) {
  streamsize __n = 0;
...
streamsize _STLP_CALL _M_read_buffered(basic_istream<_CharT, _Traits> *__that,
  ...
  if (__sentry) {
    if (__n > 0) {
      ...
      if (__buf->egptr() != __buf->gptr())
        this->_M_gcount = _M_read_buffered(
            this, __buf, __n - 1, __s, _Eq_char_bound<_Traits>(__delim),
            _Scan_for_char_val<_Traits>(__delim), false, true, false);
      else
        this->_M_gcount = _M_read_unbuffered(this, __buf, __n - 1, __s,
...
basic_istream<_CharT, _Traits>::getline(_CharT *__s, streamsize __n,
  ...
  if (__sentry) {
    if (__n > 0) {
      basic_streambuf<_CharT, _Traits> *__buf = this->rdbuf();
      this->_M_gcount =
          __buf->egptr() != __buf->gptr()
              ? _M_read_buffered(
                    this, __buf, __n - 1, __s, _Eq_char_bound<_Traits>(__delim),
                    _Scan_for_char_val<_Traits>(__delim), true, true, true)
              : _M_read_unbuffered(this, __buf, __n - 1, __s,
                                   _Eq_char_bound<_Traits>(__delim), true, true,
                                   true);
    }
  }
...
basic_istream<_CharT, _Traits>::read(char_type *__s, streamsize __n) {
  ...
  if (__sentry && !this->eof()) {
    ...
    if (__buf->gptr() != __buf->egptr())
      _M_gcount = _M_read_buffered(
          this, __buf, __n, __s, _Constant_unary_fun<bool, int_type>(false),
          _Project2nd<const _CharT *, const _CharT *>(), false, false, false);
    else
      _M_gcount = _M_read_unbuffered(this, __buf, __n, __s,
...
streamsize basic_istream<_CharT, _Traits>::readsome(char_type *__s,
  ...
  if (__sentry && !this->eof() && __nmax >= 0) {
    ...
    if (__avail == -1)
      ...
      if (__buf->gptr() != __buf->egptr())
        _M_gcount = _M_read_buffered(
            this, __buf, (min)(__avail, __nmax), __s,
            _Constant_unary_fun<bool, int_type>(false),
            _Project2nd<const _CharT *, const _CharT *>(), false, false, false);
      else
        _M_gcount = _M_read_unbuffered(
            this, __buf, (min)(__avail, __nmax), __s,
            _Constant_unary_fun<bool, int_type>(false), false, false, false);
    }
  } else {
...
void basic_istream<_CharT, _Traits>::_M_formatted_get(_CharT *__s) {
  ...
  if (__sentry) {
    ...
    streamsize __n =
        __buf->gptr() != __buf->egptr()
            ? _M_read_buffered(
                  this, __buf, __nmax, __s,
                  _Is_wspace_null<_Traits>(
                      (const ctype<_CharT> *)this->_M_ctype_facet()),
...
void _STLP_CALL _M_ignore_unbuffered(basic_istream<_CharT, _Traits> *__that,
                                     basic_streambuf<_CharT, _Traits> *__buf,
                                     _Is_Delim __is_delim, bool __extract_delim,
                                     bool __set_failbit) {
  bool __done = false;
  ios_base::iostate __status = 0;
  typedef typename basic_istream<_CharT, _Traits>::int_type int_type;
...
void _STLP_CALL _M_ignore_buffered(basic_istream<_CharT, _Traits> *__that,
  ...
  _STLP_TRY {
    while (__buf->_M_egptr() != __buf->_M_gptr() && !__at_eof &&
           !__found_delim) {
  ...
  _M_ignore_unbuffered(__that, __buf, __is_delim, __extract_delim,
                       __set_failbit);
}
...
_M_ignore_unbuffered(basic_istream<_CharT, _Traits> *__that,
                     basic_streambuf<_CharT, _Traits> *__buf, streamsize _Num,
                     _Max_Chars __max_chars, _Is_Delim __is_delim,
                     bool __extract_delim, bool __set_failbit) {
  streamsize __n = 0;
  ios_base::iostate __status = 0;
  typedef typename basic_istream<_CharT, _Traits>::int_type int_type;
...
template <class _CharT, class _Traits, class _Max_Chars, class _Is_Delim,
          class _Scan_Delim>
streamsize _STLP_CALL _M_ignore_buffered(
    basic_istream<_CharT, _Traits> *__that,
    basic_streambuf<_CharT, _Traits> *__buf, streamsize _Num,
    _Max_Chars __max_chars, _Is_Delim __is_delim, _Scan_Delim __scan_delim,
    bool __extract_delim, bool __set_failbit) {
  streamsize __n = 0;
  bool __at_eof = false;
  bool __done = false;
...
basic_istream<_CharT, _Traits>::ignore(streamsize __n) {
  ...
  if (__sentry) {
    ...
    if (__n == (numeric_limits<int>::max)()) {
      if (__buf->gptr() != __buf->egptr())
        _M_gcount = _M_ignore_buffered(
            this, __buf, __maxss, _Const_streamsize(__maxss),
            _Const_bool(false), _Project2nd<const _CharT *, const _CharT *>(),
            false, false);
      else
        _M_gcount = _M_ignore_unbuffered(this, __buf, __maxss,
                                         _Const_streamsize(__maxss),
                                         _Const_bool(false), false, false);
    } else {
      if (__buf->gptr() != __buf->egptr())
        _M_gcount = _M_ignore_buffered(
            this, __buf, __n, minus<streamsize>(), _Const_bool(false),
            _Project2nd<const _CharT *, const _CharT *>(), false, false);
      else
    ...
    if (__n == (numeric_limits<int>::max)()) {
      if (__buf->gptr() != __buf->egptr())
        _M_gcount = _M_ignore_buffered(
            ...
            _Scan_for_int_val<_Traits>(__delim), true, false);
      else
        _M_gcount = _M_ignore_unbuffered(
            this, __buf, __maxss, _Const_streamsize(__maxss),
            _Eq_int_bound<_Traits>(__delim), true, false);
    } else {
      if (__buf->gptr() != __buf->egptr())
        _M_gcount = _M_ignore_buffered(this, __buf, __n, minus<streamsize>(),
                                       _Eq_int_bound<_Traits>(__delim),
                                       _Scan_for_int_val<_Traits>(__delim),
                                       true, false);
      else
        _M_gcount =
            _M_ignore_unbuffered(this, __buf, __n, minus<streamsize>(),
                                 _Eq_int_bound<_Traits>(__delim), true, false);
    }
  }
...
_M_copy_unbuffered(basic_istream<_CharT, _Traits> *__that,
                   ...
                   _Is_Delim __is_delim, bool __extract_delim, bool __rethrow) {
...
template <class _CharT, class _Traits, class _Is_Delim, class _Scan_Delim>
streamsize _STLP_CALL _M_copy_buffered(basic_istream<_CharT, _Traits> *__that,
                                       basic_streambuf<_CharT, _Traits> *__src,
                                       basic_streambuf<_CharT, _Traits> *__dest,
                                       _Scan_Delim __scan_delim,
                                       _Is_Delim __is_delim,
  ...
  _STLP_TRY {
    while (true) {
      ...
      streamsize __n =
          __dest->sputn(__first, __extract_delim && __last != __src->_M_egptr()
                                     ? (__last - __first) + 1
                                     : (__last - __first));
      ...
      if (__avail > 0) {
        ...
        return __extracted + _M_copy_unbuffered(__that, __src, __dest,
...
template <class _CharT, class _Traits>
basic_istream<_CharT, _Traits> &
basic_istream<_CharT, _Traits>::get(basic_streambuf<_CharT, _Traits> &__dest,
                                    _CharT __delim) {
  sentry __sentry(*this, _No_Skip_WS());
  this->_M_gcount = 0;
    ...
    if (__src && __dest)
      __n = __src->egptr() != __src->gptr()
                ? _M_copy_buffered(
                      ...
                      _Constant_unary_fun<bool, int_type>(false), false, true)
                : _M_copy_unbuffered(this, __src, __dest,
                                     _Constant_unary_fun<bool, int_type>(false),
                                     false, true);
  }
...
basic_iostream<_CharT, _Traits>::basic_iostream(
    basic_streambuf<_CharT, _Traits> *__buf)
    : basic_ios<_CharT, _Traits>(), basic_istream<_CharT, _Traits>(__buf),
