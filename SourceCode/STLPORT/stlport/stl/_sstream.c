...
template <class _CharT, class _Traits, class _Alloc>
basic_stringbuf<_CharT, _Traits, _Alloc>::basic_stringbuf(
    const basic_string<_CharT, _Traits, _Alloc> &__s, ios_base::openmode __mode)
...
template <class _CharT, class _Traits, class _Alloc>
void basic_stringbuf<_CharT, _Traits, _Alloc>::str(
...
__BSB_int_type__ basic_stringbuf<_CharT, _Traits, _Alloc>::underflow() {
  return this->gptr() != this->egptr() ? _Traits::to_int_type(*this->gptr())
                                       : _Traits::eof();
}
    ...
    if (_M_mode & ios_base::out) {
      if (!(_M_mode & ios_base::in)) {
        ...
        if (this->pptr() == this->epptr()) {
          ...
          this->setg(__data_ptr, __data_ptr + __offset,
                     __data_ptr + __data_size);
...
basic_stringbuf<_CharT, _Traits, _Alloc>::xsputn(const char_type *__s,
  ...
  if ((_M_mode & ios_base::out) && __n > 0) {
    ...
    if (_M_mode & ios_base::in) {
      ...
      this->setg(__data_ptr, __data_ptr + __get_offset,
                 __data_ptr + __data_size);
      this->setp(__data_ptr, __data_ptr + __data_size);
      this->pbump((int)__data_size);
    } else {
...
template <class _CharT, class _Traits, class _Alloc>
__BSB_pos_type__ basic_stringbuf<_CharT, _Traits, _Alloc>::seekoff(
    off_type __off, ios_base::seekdir __dir, ios_base::openmode __mode) {
  bool __in = false;
  bool __out = false;

  if ((__mode & (ios_base::in | ios_base::out)) ==
  ...
  case ios_base::cur:
    __newoff =
        __in ? this->gptr() - this->eback() : this->pptr() - this->pbase();
    break;
  default:
    return pos_type(off_type(-1));
  }
...
template <class _CharT, class _Traits, class _Alloc>
__BSB_pos_type__
basic_stringbuf<_CharT, _Traits, _Alloc>::seekpos(pos_type __pos,
                                                  ios_base::openmode __mode) {
...

{
  ...
  if (this->pbase() == this->_M_Buf && this->pptr() != this->_M_Buf) {
    basic_stringbuf<_CharT, _Traits, _Alloc> *__this =
        __CONST_CAST(_Self *, this);
    __this->_M_str.append((const _CharT *)this->pbase(),
                          (const _CharT *)this->pptr());
    __this->setp(
        __CONST_CAST(_CharT *, _M_Buf),
...
template <class _CharT, class _Traits, class _Alloc>
basic_istringstream<_CharT, _Traits, _Alloc>::basic_istringstream(
    const _String &__str, ios_base::openmode __mode)
