...
#define __BF_off_type__                                                        \
  _STLP_TYPENAME_ON_RETURN_TYPE basic_filebuf<_CharT, _Traits>::off_type
#endif
...
basic_filebuf<_CharT, _Traits>::basic_filebuf()
    : basic_streambuf<_CharT, _Traits>(), _M_base(), _M_constant_width(false),
      _M_always_noconv(false), _M_int_buf_dynamic(false),
      _M_in_input_mode(false), _M_in_output_mode(false),
      _M_in_error_mode(false), _M_in_putback_mode(false), _M_int_buf(0),
      _M_int_buf_EOS(0), _M_ext_buf(0), _M_ext_buf_EOS(0),
      _M_ext_buf_converted(0), _M_ext_buf_end(0),
      _M_state(_STLP_DEFAULT_CONSTRUCTED(_State_type)),
      _M_end_state(_STLP_DEFAULT_CONSTRUCTED(_State_type)), _M_mmap_base(0),
      _M_mmap_len(0), _M_saved_eback(0), _M_saved_gptr(0), _M_saved_egptr(0),
      _M_codecvt(0), _M_width(1), _M_max_width(1) {
...
template <class _CharT, class _Traits>
__BF_int_type__ basic_filebuf<_CharT, _Traits>::pbackfail(int_type __c) {
...
template <class _CharT, class _Traits>
__BF_int_type__ basic_filebuf<_CharT, _Traits>::overflow(int_type __c) {
  ...
  while (__ibegin != __iend) {
    ...
    typename _Codecvt::result __status =
        _M_codecvt->out(_M_state, __ibegin, __iend, __inext, _M_ext_buf,
                        _M_ext_buf_EOS, __enext);
...
basic_filebuf<_CharT, _Traits>::seekoff(off_type __off,
  ...
  if (this->is_open() && (__off == 0 || (_M_constant_width &&
    ...
    // Seek relative to current position.  Complicated if we're in input mode.
      ...
      if (!_M_in_input_mode)
        ...
        streamoff __adjust =
            _M_mmap_len - (this->gptr() - (_CharT *)_M_mmap_base);
        ...
        if (__off == 0) {
          ...
          return _M_seek_return(
              _M_base._M_seek(__off - __adjust, ios_base::cur), _State_type());
        ...
        if (__iadj <= _M_ext_buf_end - _M_ext_buf) {
          ...
          streamoff __eadj =
              _M_base._M_get_offset(_M_ext_buf + __iadj, _M_ext_buf_end);

          if (__off == 0) {
            return pos_type(_M_base._M_seek(0, ios_base::cur) - __eadj);
          } else {
            return _M_seek_return(
                _M_base._M_seek(__off - __eadj, ios_base::cur), _State_type());
          }
        } else
          return pos_type(-1);
        ...
        int __epos =
            _M_codecvt->length(__state, _M_ext_buf, _M_ext_buf_end, __ipos);
        ...
        const char *__dummy;
        typename _Codecvt::result __status =
            _M_codecvt->in(__tmp_state, _M_ext_buf, _M_ext_buf + __epos,
                           __dummy, __ibegin, __ibegin + __ipos, __inext);
...
__BF_int_type__ basic_filebuf<_CharT, _Traits>::_M_underflow_aux() {
  ...
  while (true) {
    ptrdiff_t __n =
        _M_base._M_read(_M_ext_buf_end, _M_ext_buf_EOS - _M_ext_buf_end);
    ...
    typename _Codecvt::result __status =
        _M_codecvt->in(_M_end_state, _M_ext_buf, _M_ext_buf_end, __enext,
    ...
    if (__status == _Codecvt::noconv)
      return _Noconv_input<_Traits>::_M_doit(this);
             ...
             (_M_constant_width &&
              //         __inext - _M_int_buf != _M_width * (__enext -
              //         _M_ext_buf)) ||
...
bool basic_filebuf<_CharT, _Traits>::_M_unshift() {
  if (_M_in_output_mode && !_M_constant_width) {
    ...
    do {
      char *__enext = _M_ext_buf;
      __status =
          _M_codecvt->unshift(_M_state, _M_ext_buf, _M_ext_buf_EOS, __enext);
      if (__status == _Codecvt::noconv ||
...
bool basic_filebuf<_CharT, _Traits>::_M_allocate_buffers() {
  ...
  streamsize __default_bufsiz =
      ((_M_base.__page_size() + 4095UL) / _M_base.__page_size()) *
      _M_base.__page_size();
