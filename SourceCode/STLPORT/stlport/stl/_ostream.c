...
template <class _CharT, class _Traits>
bool basic_ostream<_CharT, _Traits>::_M_copy_buffered(
    basic_streambuf<_CharT, _Traits> *__from,
...
template <class _CharT, class _Traits>
bool basic_ostream<_CharT, _Traits>::_M_copy_unbuffered(
    basic_streambuf<_CharT, _Traits> *__from,
    basic_streambuf<_CharT, _Traits> *__to) {
  bool __any_inserted = false;
  ...
  while (true) {
    ...
    if (this->_S_eof(__c))
      ...
      int_type __tmp;
      _STLP_TRY { __tmp = __to->sputc(__c); }
      _STLP_CATCH_ALL {
        this->_M_handle_exception(ios_base::badbit);
        return __any_inserted;
      }
...
_M_put_num(basic_ostream<_CharT, _Traits> &__os, _Number __x) {
  ...
  if (__sentry) {
    _STLP_TRY {
      ...
      __failed = (use_facet<_NumPut>(__os.getloc()))
                     .put(ostreambuf_iterator<_CharT, _Traits>(__os.rdbuf()),
                          __os, __os.fill(), __x)
                     .failed();
    }
    _STLP_CATCH_ALL { __os._M_handle_exception(ios_base::badbit); }
  }
  if (__failed)
...
_STLP_EXPORT_TEMPLATE _STLP_DECLSPEC basic_ostream<char, char_traits<char>> &
    _STLP_CALL
...
_STLP_EXPORT_TEMPLATE _STLP_DECLSPEC basic_ostream<char, char_traits<char>> &
    _STLP_CALL
    _M_put_num(basic_ostream<char, char_traits<char>> &,
...
basic_ostream<_CharT, _Traits>::put(char_type __c) {
  ...
  if (__sentry) {
    _STLP_TRY { __failed = this->_S_eof(this->rdbuf()->sputc(__c)); }
    _STLP_CATCH_ALL { this->_M_handle_exception(ios_base::badbit); }
  }
...
basic_ostream<_CharT, _Traits>::write(const char_type *__s, streamsize __n) {
  ...
  if (__sentry) {
    _STLP_TRY { __failed = this->rdbuf()->sputn(__s, __n) != __n; }
    _STLP_CATCH_ALL { this->_M_handle_exception(ios_base::badbit); }
  }
