...
static FILE *getopthandle(lua_State *L, int inout) {
  ...
  if (p != NULL) {          /* is it a userdata ? */
    ...
    lua_pushvalue(L, 1);
    lua_remove(L, 1); /* move it to stack top */
...
static int io_readfrom(lua_State *L) { return io_fromto(L, INFILE, l_s("r")); }

static int io_writeto(lua_State *L) { return io_fromto(L, OUTFILE, l_s("w")); }
...
static int io_seek(lua_State *L) {
  ...
  static const l_char *const modenames[] = {l_s("set"), l_s("cur"), l_s("end"),
                                            NULL};
  FILE *f = (FILE *)luaL_check_userdata(L, 1, FILEHANDLE);
...
static int io_rename(lua_State *L) {
  return pushresult(
      L, rename(luaL_check_string(L, 1), luaL_check_string(L, 2)) == 0);
}
...
static int io_setloc(lua_State *L) {
  ...
  static const l_char *const catnames[] = {
      l_s("all"),     l_s("collate"), l_s("ctype"), l_s("monetary"),
...
static const luaL_reg iolib[] = {
    {l_s("appendto"), io_appendto},  {l_s("clock"), io_clock},
    {l_s("closefile"), io_close},    {l_s("date"), io_date},
    {l_s("debug"), io_debug},        {l_s("difftime"), io_difftime},
    {l_s("execute"), io_execute},    {l_s("exit"), io_exit},
    {l_s("flush"), io_flush},        {l_s("getenv"), io_getenv},
    {l_s("openfile"), io_open},      {l_s("read"), io_read},
    {l_s("readfrom"), io_readfrom},  {l_s("remove"), io_remove},
    {l_s("rename"), io_rename},      {l_s("seek"), io_seek},
    {l_s("setlocale"), io_setloc},   {l_s("time"), io_time},
    {l_s("tmpfile"), io_tmpfile},    {l_s("tmpname"), io_tmpname},
    {l_s("write"), io_write},        {l_s("writeto"), io_writeto},
